...
public class changenum2 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					changenum2 frame = new changenum2();
	...
	private void chnge() {
		...
		{JOptionPane.showMessageDialog(loginbtn, "New Number and confirm number do not match.");}
			...
			ResultSet valid=DB.check_phonetour(username, number);
			boolean check=false;
			try {
				check=valid.next();
			...
			if(check) {
                     boolean change=DB.tour_phone(number, conf, username);
				if(change) {
					this.setVisible(false);
