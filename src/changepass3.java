...
public class changepass3 extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					changepass3 frame = new changepass3();
	...
	private void changepass() {
		...
		{JOptionPane.showMessageDialog(loginbtn, "New Number and confirm number do not match.");}
			...
			Administrator st=new Administrator(username,oldpass);
			...
			if(check) {
				boolean change=DB.admin_password(oldpass, new_pass, username);
				if(change) {
					this.setVisible(false);
