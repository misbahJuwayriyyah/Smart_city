...
public class createTourAcc extends JFrame {
	...
	private void create() {
		...
		{JOptionPane.showMessageDialog(loginbtn, "The password and confirm password do not match.");}
			...
			Tour st=new Tour(fullname,lastname,username,cpass,cnumber);
			ResultSet check=DB.tour_check(username);
			boolean ans=false;
			boolean valid=false;
			try {
				 ans=check.next();
			...
			if(ans) {
				 ...
				 valid=DB.tourist_signup(st);
					if(valid) {
						this.setVisible(false);
						tourist2 s1=new tourist2();
						s1.setVisible(true);
					}else {
